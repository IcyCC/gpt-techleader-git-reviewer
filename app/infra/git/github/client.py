...
from app.models.git import MergeRequest, FileDiff, MergeRequestState, ChangeType
from app.models.comment import Comment, CommentType, CommentPosition
...
class GitHubClient(GitClientBase):
    ...
    async def _request(self, method: str, url: str, **kwargs) -> Any:
        ...
        async with aiohttp.ClientSession(timeout=self.timeout) as session:
            async with session.request(method, f"{self.github_api_url}{url}", headers=headers, **kwargs) as response:
    ...
    async def get_merge_request(self, owner: str, repo: str, mr_id: str) -> MergeRequest:
        ...
        try:
            ...
            pr_data = await self._request(
                "GET", 
                f"/repos/{owner}/{repo}/pulls/{mr_id}"
            )
            
            # 获取文件变更
            files_data = await self._request(
                "GET",
                f"/repos/{owner}/{repo}/pulls/{mr_id}/files"
            )
    ...
    async def create_comment(self, owner: str, repo: str, comment: Comment):
        ...
        try:
            ...
            if comment.comment_type == CommentType.FILE:
                ...
                pr_data = await self._request(
                    "GET",
                    f"/repos/{owner}/{repo}/pulls/{comment.mr_id}"
                )
    ...
    async def resolve_review_thread(self, owner: str, repo: str, mr_id: str, thread_id: str):
        pass
    ...
    async def _convert_github_comment_to_model(
        ...
        if "path" in comment_data and "line" in comment_data:
            position = CommentPosition(
                ...
                new_line_number=comment_data["line"]
            )
        ...
        return Comment(
            ...
            mr_id=str(mr.mr_id),
            position=position,
            reply_to=str(comment_data["in_reply_to_id"]) if comment_data.get("in_reply_to_id") else None
        )

    async def list_comments(self, owner: str, repo: str, mr: MergeRequest) -> List[Comment]:
        """获取评论列表"""
        ...
        for comment_data in comments_data:
            comment = await self._convert_github_comment_to_model(owner, repo, mr, comment_data)
    ...
    async def get_comment(self, owner: str, repo: str, mr: MergeRequest, comment_id: str) -> Comment:
        ...
        return await self._convert_github_comment_to_model(owner, repo, mr, comment_data)
