...
class ReviewPipeline(BaseModel):
    ...
    def get_prompt_template(self, lang: str = "中文") -> Dict[str, str]:
        ...
        templates = {
            "中文": {
                "system_role": "你是代码审查助手。提供简短精确的建议。只关注关键问题。只给出需要明确修改的建议。",
                "json_format": (
                    "使用以下JSON格式返回结果:\n"
                    "{\n"
                    '  "summary": "简短总结",\n'
            ...
            "english": {
                "system_role": "You are a code review assistant. Provide brief, precise suggestions. Focus only on critical issues. Only provide suggestions that need to be clearly modified.",
                "json_format": (
