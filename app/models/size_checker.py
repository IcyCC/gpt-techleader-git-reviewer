...
class SizeChecker:
    ...
    def check_mr_size(self, mr: MergeRequest) -> Optional[ReviewResult]:
        ...
        if len(mr.file_diffs) > self.settings.MAX_FILES_PER_MR:
            logger.warning(f"MR 文件数量过多: {len(mr.file_diffs)} > {self.settings.MAX_FILES_PER_MR}")
            return ReviewResult(
                mr_id=mr.mr_id,
    ...
    def check_files_size(self, mr: MergeRequest) -> Tuple[List[FileDiff], List[FileDiff]]:
    ...
    def create_large_file_comment(self, mr: MergeRequest, file_diff: FileDiff, lines: int) -> Comment:
        """创建大文件评论"""
        return Comment(
    ...
    def create_large_files_summary(self, large_files: List[FileDiff]) -> str:
        """创建大文件总结"""
        summary = f"以下文件超过了最大行数限制 ({self.settings.MAX_LINES_PER_FILE} 行)：\n"
