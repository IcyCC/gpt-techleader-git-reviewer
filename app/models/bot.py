...
from .pipeline import ReviewPipeline, StaticAnalysisPipeline, LogicReviewPipeline
...
class Bot(BaseModel):
    ...
    async def _handle_review_mr(self, mr: MergeRequest) -> Tuple[ReviewResult, List[Comment]]:
        settings = get_settings()
        rate_limiter = RateLimiter()
        # 检查 MR 处理次数限制
        if not await rate_limiter.check_and_increment(rate_limiter.get_mr_reviews_key(), settings.MAX_MR_REVIEWS_PER_HOUR):
            return ReviewResult(
                mr_id=mr.mr_id,
        ...
        if large_files:
            for file_diff in large_files:
                comment = size_checker.create_large_file_comment(mr, file_diff, file_diff.diff_content.count('\n') + 1)
    ...
    async def handle_comment(self, mr: MergeRequest, comment: Comment) -> Optional[Comment]:
        """处理评论回复"""
        ...
        await self._post_comment(git_client, mr, comment)
        if resolved:
            await git_client.resolve_review_thread(mr.owner, mr.repo, mr.mr_id, comment.comment_id)
        return comment


    @staticmethod
    async def _post_comment(git_client: GitHubClient, mr: MergeRequest, comment: Comment):
        """发布评论到 Git 平台"""
        try:
