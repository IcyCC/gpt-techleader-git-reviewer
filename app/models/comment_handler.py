...
class CommentHandler:
    ...
    async def handle_comment(self, mr: MergeRequest, comment: Comment) -> Tuple[Comment, bool]:
        """处理评论回复"""
        ...
        if not await rate_limiter.check_and_increment(key, self.settings.MAX_COMMENT_REPLIES):
            raise Exception(f"此评论已达到最大回复次数限制 ({self.settings.MAX_COMMENT_REPLIES})")
    ...
    async def _get_discussion_context(self, mr: MergeRequest, comment: Comment) -> Optional[Discussion]:
        """获取讨论上下文"""
        discussions = await self.discussion_service.build_discussions(mr.owner, mr.repo, mr.mr_id)
    ...
    def _get_discussion_file(self, mr: MergeRequest, discussion) -> Optional[FileDiff]:
        ...
        if discussion.comments:
            ...
            if first_comment.position and first_comment.position.file_path:
                return next(
                    (diff for diff in mr.file_diffs if diff.file_name == first_comment.position.file_path),
                    None
                )
        return None
